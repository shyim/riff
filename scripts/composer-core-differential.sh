...
if [[ ! -x "$php_bin" || ! -x "$riff_bin" ]]; then
    echo "Build PHP and riff before running core differential tests." >&2
    exit 1
fi
...
run_pair() {
    ...
    reference_code=$?
    COMPOSER_HOME="$tmp_dir/home-candidate" "$riff_bin" \
    ...
    if [[ $reference_code -ne $candidate_code ]]; then
        record_failure "$case_name" "exit code: Composer=$reference_code riff=$candidate_code"
        printf '%s\n' '--- Composer output ---' >&2
...
compare_json_projection() {
    ...
    local candidate_json=$tmp_dir/$case_name-riff.json
...
if [[ $reference_autoload_code -ne $candidate_autoload_code \
    ...
    record_failure autoload-runtime \
        "Composer[$reference_autoload_code]=$reference_autoload riff[$candidate_autoload_code]=$candidate_autoload"
else
