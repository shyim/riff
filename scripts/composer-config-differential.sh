...
read_fixture=$repo_root/crates/riff/tests/fixtures/composer-parity/config/read/composer.json
...
if [[ ! -x "$riff_bin" ]]; then
    echo "Missing riff binary at $riff_bin; run 'make build' first." >&2
    exit 1
fi
...
compare_outputs() {
    ...
    local candidate_normalized=$tmp_dir/$case_name-riff.normalized
    ...
    if [[ $reference_code -ne $candidate_code ]]; then
        printf 'FAIL %-30s exit code: Composer=%s riff=%s\n' \
...
run_read() {
    ...
    reference_code=$?
    COMPOSER_HOME="$home" "$riff_bin" config -d "$candidate_dir" "$@" > "$candidate_output" 2>&1
    candidate_code=$?
    set -e
...
compare_json_file() {
    ...
    jq -S . "$candidate_path" > "$tmp_dir/$case_name-riff-${relative_path//\//-}"
    if ! diff -u \
        "$tmp_dir/$case_name-reference-${relative_path//\//-}" \
        "$tmp_dir/$case_name-riff-${relative_path//\//-}"; then
...
candidate_output=$tmp_dir/global-riff.out
set +e
...
reference_code=$?
COMPOSER_HOME="$candidate_dir" "$riff_bin" config --global vendor-dir global-vendor > "$candidate_output" 2>&1
candidate_code=$?
set -e
global_passed=true
...
for file in config.json auth.json; do
    ...
    jq -S . "$candidate_dir/$file" > "$tmp_dir/global-riff-$file"
    if ! diff -u "$tmp_dir/global-reference-$file" "$tmp_dir/global-riff-$file"; then
        printf 'FAIL %-30s %s differs\n' global "$file" >&2
        failures=$((failures + 1))
        global_passed=false
    fi
