...
riff_bin=${RIFF_BIN:-$repo_root/target/debug/riff}
export RIFF_PHP="$php_bin"

if [[ ! -x "$php_bin" || ! -x "$riff_bin" || ! -f "$composer_dir/vendor/autoload.php" ]]; then
    echo "Build PHP, riff, and the Composer reference before running operation differential tests." >&2
    exit 1
fi
...
run_pair() {
    ...
    local candidate_output=$tmp_dir/$case_name-riff.out
    ...
    reference_code=$?
    COMPOSER_HOME="$tmp_dir/home-candidate" "$riff_bin" "$command" -d "$candidate_dir" "$@" \
        > "$candidate_output" 2> "$tmp_dir/$case_name-riff.err"
    candidate_code=$?
    set -e
    ...
    if [[ $reference_code -ne 0 || $candidate_code -ne 0 ]]; then
        ...
        printf '%s\n' '--- riff output ---' >&2
        cat "$candidate_output" "$tmp_dir/$case_name-riff.err" >&2
        fail "$case_name" "exit code: Composer=$reference_code riff=$candidate_code"
    fi
...
sort "$tmp_dir/show-names-riff.out" > "$tmp_dir/show-names-riff.sorted"
diff -u "$tmp_dir/show-names-reference.sorted" "$tmp_dir/show-names-riff.sorted" \
    || fail show-names-output "installed names differ"
printf 'PASS %-32s installed names\n' show-names-output

run_pair why-transitive why fixture/base
for output in "$tmp_dir/why-transitive-reference.out" "$tmp_dir/why-transitive-riff.out"; do
...
sort "$tmp_dir/show-locked-riff.out" > "$tmp_dir/show-locked-riff.sorted"
diff -u "$tmp_dir/show-locked-reference.sorted" "$tmp_dir/show-locked-riff.sorted" \
...
for output in "$tmp_dir/why-locked-reference.out" "$tmp_dir/why-locked-riff.out"; do
...
jq -S '.installed | sort_by(.name) | map({name, version, latest, status: .["latest-status"]})' \
    "$tmp_dir/outdated-json-riff.out" > "$tmp_dir/outdated-json-riff.projected"
diff -u "$tmp_dir/outdated-json-reference.projected" "$tmp_dir/outdated-json-riff.projected" \
...
reference_code=$?
COMPOSER_HOME="$tmp_dir/home-candidate" "$riff_bin" outdated --strict -d "$candidate_dir" >/dev/null 2>&1
candidate_code=$?
set -e
[[ $reference_code -eq 1 && $candidate_code -eq 1 ]] \
    || fail outdated-strict "expected exit 1, Composer=$reference_code riff=$candidate_code"
...
[[ $(jq -r '.installed | map(.name) | join(",")' "$tmp_dir/outdated-major-reference.out") == 'fixture/tool' \
    && $(jq -r '.installed | map(.name) | join(",")' "$tmp_dir/outdated-major-riff.out") == 'fixture/tool' ]] \
...
[[ $(jq -r '.installed | map(.name) | join(",")' "$tmp_dir/outdated-ignore-reference.out") == 'fixture/app' \
    && $(jq -r '.installed | map(.name) | join(",")' "$tmp_dir/outdated-ignore-riff.out") == 'fixture/app' ]] \
