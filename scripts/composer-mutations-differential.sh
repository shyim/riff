...
if [[ ! -x "$php_bin" || ! -x "$riff_bin" || ! -f "$composer_dir/vendor/autoload.php" ]]; then
    echo "Build PHP, riff, and the Composer reference before running mutation differential tests." >&2
    exit 1
fi
...
compare_projection() {
    ...
    jq -S "$filter" "$candidate_file" > "$tmp_dir/$case_name-riff.json"
    diff -u "$tmp_dir/$case_name-reference.json" "$tmp_dir/$case_name-riff.json" \
...
[[ $reference_runtime == "$candidate_runtime" && $candidate_runtime == 'base-app:tests' ]] \
    || fail require-autoload-runtime "Composer=$reference_runtime riff=$candidate_runtime"
...
[[ $reference_runtime == "$candidate_runtime" && $candidate_runtime == 'base-app' ]] \
    || fail dump-authoritative-runtime "Composer=$reference_runtime riff=$candidate_runtime"
...
reference_code=$?
COMPOSER_HOME="$tmp_dir/home-candidate" "$riff_bin" require -d "$candidate_dir" 'missing/package:^1.0' \
    > "$tmp_dir/require-failure-riff.out" 2>&1
candidate_code=$?
set -e
