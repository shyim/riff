...
fixtures=$repo_root/crates/riff/tests/fixtures/composer-parity/validate
...
if [[ ! -f "$composer_dir/bin/composer" || ! -f "$composer_dir/vendor/autoload.php" ]]; then
    ...
    echo "Run: $riff_bin install --no-scripts --no-audit -n -d $composer_dir" >&2
    exit 1
fi
...
run_directory_case() {
    ...
    reference_code=$?
    (cd "$case_dir" && "$riff_bin" validate "$@") >"$candidate_output" 2>&1
    candidate_code=$?
    set -e
    ...
    if [[ $reference_code -ne $candidate_code ]]; then
        printf 'FAIL %-24s exit code: Composer=%s riff=%s\n' \
