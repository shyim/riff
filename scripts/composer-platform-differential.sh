...
if [[ ! -x "$php_bin" || ! -x "$riff_bin" ]]; then
    echo "Build riff and provide a PHP CLI before platform differential tests." >&2
    exit 1
fi
...
"$riff_bin" --php "$php_bin" show --platform --format=json \
...
mkdir -p "$project"
printf '%s\n' '{"config":{"platform":{"php":"8.2.0","ext-json":false,"ext-riff-test":"1.2.3"}}}' \
    > "$project/composer.json"
"$riff_bin" --php "$php_bin" show --platform --format=json -d "$project" \
    > "$tmp_dir/overrides.json"

jq -e '.platform | any(.name == "php" and .version == "8.2.0")' "$tmp_dir/overrides.json" >/dev/null
jq -e '.platform | any(.name == "ext-riff-test" and .version == "1.2.3")' "$tmp_dir/overrides.json" >/dev/null
