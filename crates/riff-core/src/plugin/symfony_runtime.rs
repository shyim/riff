...
pub(super) fn register(registrar: &mut PluginRegistrar) {
    let plugin = Arc::new(SymfonyRuntimePlugin);
...
impl EventListener for SymfonyRuntimePlugin {
    fn handle(&self, event: &dyn RiffEvent, riff: &Riff) -> anyhow::Result<i32> {
        ...
        self.post_autoload_dump(&riff.vendor_dir(), &riff.working_dir, &riff.manifest)?;
...
impl SymfonyRuntimePlugin {
    fn post_autoload_dump(
        ...
        let runtime_extra = manifest.extra.get("runtime");
        ...
        let template_content = if let Some(template_path) = autoload_template_path {
            ...
            std::fs::read_to_string(&resolved_path).map_err(|_| {
                crate::error::RiffError::Config(format!(
