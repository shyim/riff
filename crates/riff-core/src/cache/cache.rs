...
impl Cache {
    ...
    pub fn is_enabled(&self) -> bool {
        ...
        if !self.root.exists() && fs::create_dir_all(&self.root).is_err() {
            return false;
        }
    ...
    pub fn gc(&self, ttl: Duration) -> io::Result<u64> {
        self.gc_with_max_size(ttl, u64::MAX)
    }

    /// Garbage collect expired entries and enforce a maximum cache size.
    ///
    /// Composer first removes entries older than the TTL and then evicts the
    /// least recently accessed remaining files until the cache fits within the
    /// configured size limit.
    pub fn gc_with_max_size(&self, ttl: Duration, max_size: u64) -> io::Result<u64> {
        if !self.enabled || self.read_only {
            return Ok(0);
        }
            ...
            .filter_map(Result::ok)
        {
            ...
            remaining.push((accessed, path.to_path_buf(), metadata.len()));
        }
...
mod tests {
    ...
    #[test]
    fn composer_cache_clear_removes_all_entries() {
        let temp = TempDir::new().unwrap();
        let cache = Cache::new(temp.path().to_path_buf());
        for index in 0..4 {
            cache
                .write(&format!("cached.file{index}.zip"), &[b'0'; 1_000])
                .unwrap();
        }
    ...
    #[test]
    fn composer_cache_removes_outdated_files() {
        let temp = TempDir::new().unwrap();
        let cache = Cache::new(temp.path().to_path_buf());
        cache.write("cached.file1.zip", &[b'0'; 1_000]).unwrap();
        thread::sleep(StdDuration::from_millis(20));
        cache.write("cached.file0.zip", &[b'0'; 1_000]).unwrap();

        assert_eq!(
            cache
                .gc_with_max_size(Duration::from_millis(10), u64::MAX)
                .unwrap(),
            1_000
        );
        assert!(!cache.has("cached.file1.zip"));
        assert!(cache.has("cached.file0.zip"));
    }

    #[test]
    fn composer_cache_removes_oldest_files_until_size_limit() {
        let temp = TempDir::new().unwrap();
        let cache = Cache::new(temp.path().to_path_buf());
        for index in 0..4 {
            cache
        ...
        let freed = cache.gc_with_max_size(Duration::MAX, 1_500).unwrap();

        assert_eq!(freed, 3_000);
        assert_eq!(cache.size().unwrap(), 1_000);
        assert!(cache.has("cached.file3.zip"));
    }
