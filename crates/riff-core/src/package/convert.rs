...
use crate::json::{
    LockArchive, LockAuthor, LockAutoload, LockDist, LockFunding, LockSource, LockedPackage,
};
...
impl From<&LockedPackage> for Package {
    fn from(lp: &LockedPackage) -> Self {
        ...
        if let Some(ref src) = lp.source {
            let mut source = Source::new(&src.source_type, &src.url, &src.reference);
            source.mirrors.clone_from(&src.mirrors);
        ...
        if let Some(ref dist) = lp.dist {
            ...
            d.mirrors.clone_from(&dist.mirrors);
        ...
        pkg.archive = lp.archive.as_ref().map(|archive| super::ArchiveConfig {
...
impl From<&Package> for LockedPackage {
    fn from(pkg: &Package) -> Self {
        ...
        LockedPackage {
            ...
            abandoned,
            archive: pkg.archive.as_ref().map(|archive| LockArchive {
                name: archive.name.clone(),
                exclude: archive.exclude.clone(),
            }),
        }
...
mod tests {
    ...
    #[test]
    fn composer_array_dumper_dumps_abandoned_flag() {
        let mut package = Package::new("dummy/pkg", "1.0.0.0");
        package.pretty_version = Some("1.0.0".into());
        package.abandoned = Some(crate::package::Abandoned::Yes);

        let dumped = serde_json::to_value(LockedPackage::from(&package)).unwrap();

        assert_eq!(dumped["abandoned"], true);
    }

    #[test]
    fn composer_array_dumper_dumps_abandoned_replacement() {
        let mut package = Package::new("dummy/pkg", "1.0.0.0");
        package.pretty_version = Some("1.0.0".into());
        package.abandoned = Some(crate::package::Abandoned::Replacement(
            "foo/bar".to_string(),
        ));

        let dumped = serde_json::to_value(LockedPackage::from(&package)).unwrap();

        assert_eq!(dumped["abandoned"], "foo/bar");
    }
