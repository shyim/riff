        ...
        for pkg_id in installed_pkgs {
            ...
            if let Some(package) = pool.package(pkg_id) {
                ...
                for alias_id in aliases {
                    if let Some(PoolEntry::Alias(alias)) = pool.entry(alias_id) {
                        result.aliases.push(alias.clone());
                    }
                }
            }
...
mod tests {
    ...
    #[test]
    fn composer_solver_surfaces_policy_removed_locked_packages_as_problems() {
        ...
        let mut request = Request::new();
        request.fix(package.clone());
        request.lock(package);

        let policy = Policy::new();
        let problems = Solver::new(&pool, &policy).solve(&request).unwrap_err();

        assert_eq!(problems.len(), 1);
        assert_eq!(problems.problems()[0].rules.len(), 1);
        let rule = &problems.problems()[0].rules[0];
        assert_eq!(rule.rule_type, RuleType::LockedFilterListRemoved);
        assert_eq!(
            rule.locked_package
                .as_ref()
