...
pub struct ProblemRule {
    ...
    /// Exact locked package retained for dependency-policy diagnostics.
    pub locked_package: Option<Arc<Package>>,
}
...
impl Problem {
    ...
    pub fn add_rule(&mut self, rule: &Rule) {
        self.rules.push(ProblemRule {
            ...
            locked_package: rule.locked_package().cloned(),
        });
    }
    ...
    /// Explain why a locked package is missing from a policy-filtered pool.
    pub fn missing_locked_package_reason(pool: &Pool, package: &Package) -> (String, String) {
        let prefix = format!(
        ...
        let Some(entries) =
            pool.filter_list_removed_entries(&package.name, package.version.as_str())
        else {
            return (
                prefix,
                "is missing from the package pool and cannot be installed.".to_owned(),
            );
        };
        ...
        let details = entries
            .iter()
            .map(|entry| {
                let mut detail = format!("was flagged as {}", entry.list_name);
                ...
                detail.push_str(&format!(
                    ". Add the package to \"policy.{}.ignore\" or disable blocking with \"policy.{}.block\".",
...
fn describe_rule(pool: &Pool, rule: &ProblemRule) -> String {
    match rule.rule_type {
        ...
        RuleType::LockedFilterListRemoved => rule.locked_package.as_ref().map_or_else(
...
mod tests {
    ...
    #[test]
    fn composer_problem_explains_policy_removed_locked_packages() {
        let package = Package::new("vendor/malware", "1.0.0");
        let mut pool = Pool::new();
        pool.mark_filter_list_removed(
            &package.name,
            package.version.as_str(),
            vec![crate::filter_list::FilterListEntry {
                package_name: package.name.clone(),
                list_name: "malware".into(),
                constraint: "*".into(),
                url: Some("https://example.org/malware/vendor-malware".into()),
                reason: Some("looks suspicious".into()),
                id: Some("PKG-1".into()),
                source: Some("aikido".into()),
            }],
        );

        let (prefix, suffix) = Problem::missing_locked_package_reason(&pool, &package);
        assert_eq!(prefix, "- Package vendor/malware 1.0.0 (in the lock file) ");
        for detail in [
            ...
            assert!(suffix.contains(detail), "missing detail: {detail}");
        }
    }
}
