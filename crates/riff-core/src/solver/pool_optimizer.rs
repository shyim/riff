...
pub struct PoolOptimizer<'a> {
    ...
    /// Exact package candidates rejected by dependency policy.
    policy_excluded_packages: PackageIdSet,

    /// Maps base package IDs to their alias package IDs
    aliases_per_package: HashMap<PackageId, Vec<PackageId>>,

    /// Package groups whose versions are represented by each retained optimizer entry.
    version_inventory_groups: Vec<Vec<PackageId>>,

    /// Inventory group memberships keyed by package ID.
    version_inventory_memberships: HashMap<PackageId, SmallVec<[usize; 2]>>,
...
impl<'a> PoolOptimizer<'a> {
    ...
    pub fn new(policy: &'a Policy) -> Self {
        Self {
            ...
            policy_excluded_packages: PackageIdSet::default(),
            aliases_per_package: HashMap::new(),
            version_inventory_groups: Vec::new(),
            version_inventory_memberships: HashMap::new(),
    ...
    pub fn optimize(&mut self, request: &Request, pool: &Pool) -> Pool {
        ...
        self.policy_excluded_packages.reset(pool.len());
        self.aliases_per_package.clear();
        self.version_inventory_groups.clear();
        self.version_inventory_memberships.clear();
    ...
    fn prepare<'b>(
        ...
        // Composer filters candidates rejected by security and package policy
        // before optimizing the pool. Preserve the same behavior here. The
        // request keeps Arc identities from this pool, so match those exact
        // candidates and their aliases rather than excluding a whole name.
        for excluded in &request.excluded_packages {
            for id in pool.packages_by_name(&excluded.name) {
                let matches = pool.entry(id).is_some_and(|entry| match entry {
                ...
                self.policy_excluded_packages.insert(id);
                self.packages_to_remove.insert(id);
                if let Some(base_id) = pool.get_alias_base(id) {
                    self.policy_excluded_packages.insert(base_id);
                    self.packages_to_remove.insert(base_id);
                    if let Some(alias_ids) = self.aliases_per_package.get(&base_id) {
                        for &alias_id in alias_ids {
                            self.policy_excluded_packages.insert(alias_id);
                            self.packages_to_remove.insert(alias_id);
                        }
                    }
                } else if let Some(alias_ids) = self.aliases_per_package.get(&id) {
                    for &alias_id in alias_ids {
                        self.policy_excluded_packages.insert(alias_id);
                        self.packages_to_remove.insert(alias_id);
                    }
        ...
        self.record_alias_version_inventories(pool, request);

        // A base dev version can fail an active constraint while one of its
        // branch aliases satisfies it (for example dev-main as 1.1.x-dev for
        // a ^1.0 requirement). Ranking/removing the base by its own version
        // would then also remove the matching alias, so keep each aliased
        // package set intact for the solver to rank using alias versions.
        for (&base_id, alias_ids) in &self.aliases_per_package {
    ...
    fn record_alias_version_inventories(&mut self, pool: &Pool, request: &Request) {
        let alias_bases = self.aliases_per_package.keys().copied().collect::<Vec<_>>();
        for base_id in alias_bases {
            let Some(base) = pool.package(base_id) else {
                continue;
            };
            let base_fingerprint = Self::dependency_fingerprint(base);
            let mut candidate_bases = request
                .all_requires()
                .filter(|(name, _)| name.eq_ignore_ascii_case(&base.name))
                .flat_map(|(name, constraint)| {
                    pool.what_provides_direct_only(name, Some(constraint))
                })
                .filter(|id| !pool.is_alias(*id))
                .filter(|id| {
                    pool.package(*id).is_some_and(|package| {
                        Self::dependency_fingerprint(package) == base_fingerprint
                    })
                })
                .collect::<Vec<_>>();
            if candidate_bases.is_empty() {
                candidate_bases.push(base_id);
            }
            candidate_bases.sort_unstable();
            candidate_bases.dedup();

            let member_ids = candidate_bases
                .iter()
                .copied()
                .flat_map(|id| {
                ...
                .collect::<Vec<_>>();
            self.record_version_inventory_group(member_ids);
        }
    }

    fn record_version_inventory_group(&mut self, mut member_ids: Vec<PackageId>) {
        ...
        let group_id = self.version_inventory_groups.len();
        for &id in &member_ids {
            self.version_inventory_memberships
                .entry(id)
                .or_default()
                .push(group_id);
        }
        self.version_inventory_groups.push(member_ids);
    }
    ...
    fn optimize_by_identical_dependencies(
        ...
        for id in pool.all_package_ids() {
            ...
            // Dependency policy exclusions must never be selected as the
            // representative retained from an otherwise-identical group.
            if self.policy_excluded_packages.contains(id) {
                continue;
            }
            ...
            // A package participates in the optimizer group for its concrete
            // name and every virtual name it replaces/provides. Keeping these
            // groups separate is important for diagnostics: one retained
            // package can represent versions removed from several name groups.
            let mut matching_names = SmallVec::<
                [(
                    Cow<'_, str>,
                    Option<&ConstraintBucket>,
                    Option<&ConstraintBucket>,
                ); 2],
            >::new();
            for raw_name in std::iter::once(pkg.name.as_str())
                .chain(pkg.replace.keys().map(|name| name.as_str()))
                .chain(pkg.provide.keys().map(|name| name.as_str()))
            {
                let package_name = canonical_package_name(raw_name);
                if matching_names
                    .iter()
                    .any(|(seen, _, _)| seen.as_ref() == package_name.as_ref())
                {
                    continue;
                }
                let matching_requires = require_constraints.get(package_name.as_ref());
                let matching_conflicts = conflict_constraints.get(package_name.as_ref());
                if matching_requires.is_some() || matching_conflicts.is_some() {
                    matching_names.push((package_name, matching_requires, matching_conflicts));
                }
            }
            ...
            let dependency_fingerprint = Self::dependency_fingerprint(pkg);

            for (package_name, matching_requires, matching_conflicts) in matching_names {
                let mut group_hasher = foldhash::quality::FixedState::default().build_hasher();
                let mut matched_constraint = false;

                if let Some(constraints) = matching_requires {
                    for &constraint_id in constraints {
                        if Self::indexed_constraint_matches(
                            ...
                            (constraint_id << 1).hash(&mut group_hasher);
                            matched_constraint = true;
                        }
                    }
                }
                if let Some(constraints) = matching_conflicts {
                    for &constraint_id in constraints {
                        if Self::indexed_constraint_matches(
                            version_parser,
                            parsed_constraints,
                            ...
                            ((constraint_id << 1) | 1).hash(&mut group_hasher);
                            matched_constraint = true;
                        }
                    }
                }
        ...
        self.record_group_version_inventories(&groups);
    ...
    fn record_group_version_inventories(
        &mut self,
        groups: &HashMap<DependencyGroupKey<'_>, SmallVec<[PackageId; 4]>>,
    ) {
    ...
    fn keep_package(&mut self, pool: &Pool, id: PackageId) {
        ...
        if let Some(aliases) = self.aliases_per_package.get(&id).cloned() {
            for alias_id in aliases {
                if !self.policy_excluded_packages.contains(alias_id) {
        ...
        if let Some(base_id) = pool.get_alias_base(id) {
            if !self.policy_excluded_packages.contains(base_id) {
    ...
    fn apply_removals_to_pool(&self, original_pool: &Pool) -> Pool {
        ...
        for (name, (total, removed)) in &pkg_counts {
            let removed_by_policy = original_pool
                .packages_by_name(name)
                .into_iter()
                .all(|id| self.policy_excluded_packages.contains(id));
            if *removed == *total && *total > 0 && !removed_by_policy {
        ...
        new_pool.copy_filter_list_removals_from(original_pool);
        for (name, stability) in original_pool.stability_flags() {
            new_pool.add_stability_flag(name, stability);
        }
        let mut old_to_new = HashMap::with_capacity(original_pool.len());
            ...
            if let Some(entry) = original_pool.entry(id) {
                match entry {
                    PoolEntry::Package(pkg) => {
                        ...
                        let bypass_stability = self.irremovable_packages.contains(id)
                            || is_platform_package(&pkg.name)
                            || !pkg.replace.is_empty()
                            || !pkg.provide.is_empty();

                        if bypass_stability {
                            let new_id = new_pool
                                .add_package_arc_bypass_stability(Arc::clone(pkg), repo_name);
                            old_to_new.insert(id, new_id);
                        } else {
                            let new_id = new_pool.add_package_arc(Arc::clone(pkg), repo_name);
                            old_to_new.insert(id, new_id);
                        }
...
mod tests {
    ...
    #[test]
    fn composer_pool_optimizer_applies_request_constraints_and_policy_preferences() {
        fn optimized_versions(prefer_lowest: bool) -> Vec<String> {
            let mut pool = Pool::new();
            for version in ["1.0.0", "1.1.0", "2.0.0"] {
                let mut package = Package::new("package/a", version);
                package.require.insert("package/b".into(), "^1.0".into());
                pool.add_package(package);
            }
            pool.add_package(Package::new("package/b", "1.0.0"));
            let mut request = Request::new();
            request.require("package/a", "^1.0");
            let policy = Policy::new().prefer_lowest(prefer_lowest);
            let optimized = PoolOptimizer::new(&policy).optimize(&request, &pool);
            optimized
                .packages_by_name("package/a")
                .into_iter()
                .filter_map(|id| optimized.entry(id))
    ...
    #[test]
    fn composer_pool_optimizer_records_versions_from_replacement_name_groups() {
        fn replacing(name: &str, version: &str) -> Package {
            let mut package = Package::new(name, version);
            package.replace.insert("package/b".into(), "^1.0".into());
            package
        }

        let mut pool = Pool::new();
        pool.add_package(replacing("package/a", "1.0.0"));
        pool.add_package(replacing("package/a", "1.1.0"));
        pool.add_package(replacing("package/c", "1.5.0"));
        pool.add_package(replacing("package/c", "1.6.0"));
        let mut request = Request::new();
        request.require("package/a", "^1.0");
        request.require("package/b", "^1.0");

        let policy = Policy::new();
        let optimized = PoolOptimizer::new(&policy).optimize(&request, &pool);
        let retained_a = optimized
            .packages_by_name("package/a")
            .into_iter()
            .find(|&id| {
                optimized
                    .entry(id)
                    .is_some_and(|entry| entry.version() == "1.1.0")
            })
            .unwrap();
        let versions = optimized.removed_versions_by_package(retained_a).unwrap();

        for version in ["1.0.0", "1.1.0", "1.5.0", "1.6.0"] {
            assert!(
                versions.contains_key(version),
                "missing {version}: {versions:?}"
            );
        }
    }

    #[test]
    fn composer_pool_optimizer_records_alias_family_versions_for_each_retained_entry() {
        let mut pool = Pool::new();
        let package110 = Arc::new(Package::new("package/a", "1.1.0"));
        let package110_id = pool.add_package_arc(Arc::clone(&package110), None);
        let alias110_id = pool.add_alias_package(AliasPackage::new(
            Arc::clone(&package110),
            "1.1.x-dev".into(),
            "1.1.x-dev".into(),
        ));
        let sibling_alias_id = pool.add_alias_package(AliasPackage::new(
            Arc::clone(&package110),
            "1.1.x-dev".into(),
            "1.1.x-dev".into(),
        ));
        let package111 = Arc::new(Package::new("package/a", "1.1.1"));
        pool.add_package_arc(Arc::clone(&package111), None);
        pool.add_alias_package(AliasPackage::new(
            package111,
            "1.1.x-dev".into(),
            "1.1.x-dev".into(),
        ));
        let mut request = Request::new();
        request.require("package/a", "^1.1");

        let policy = Policy::new();
        let optimized = PoolOptimizer::new(&policy).optimize(&request, &pool);
        for old_id in [package110_id, alias110_id, sibling_alias_id] {
            let old_entry = pool.entry(old_id).unwrap();
            let retained_id = optimized
                .packages_by_name("package/a")
                .into_iter()
                .find(|&id| {
                    optimized.entry(id).is_some_and(|entry| {
                        entry.version() == old_entry.version()
                            && entry.is_alias() == old_entry.is_alias()
                    })
                })
                .unwrap();
            let versions = optimized.removed_versions_by_package(retained_id).unwrap();
            assert_eq!(
                versions.keys().map(String::as_str).collect::<Vec<_>>(),
                ["1.1.0", "1.1.1", "1.1.x-dev"]
            );
        }
    }

    #[test]
    fn optimizer_preserves_branch_alias_that_outranks_a_stable_version() {
        let mut pool = Pool::with_minimum_stability(Stability::Dev);
        pool.add_package(Package::new("vendor/a", "1.0.0"));

        let dev = Arc::new(Package::new("vendor/a", "dev-main"));
        pool.add_package_arc(dev.clone(), None);
        pool.add_alias_package(AliasPackage::new(
            dev,
            "1.1.x-dev".to_string(),
            "1.1.x-dev".to_string(),
        ...
        let mut request = Request::new();
        request.require("vendor/a", "^1.0@dev");

        let policy = Policy::new().prefer_stable(false);
        let mut optimizer = PoolOptimizer::new(&policy);
        let optimized = optimizer.optimize(&request, &pool);
        let candidates = optimized.what_provides("vendor/a", Some("^1.0@dev"));
        let selected = policy.select_preferred(&optimized, &candidates);

        assert!(selected.first().is_some_and(|id| {
            optimized
                .entry(*id)
                .is_some_and(|entry| entry.is_alias() && entry.version() == "1.1.x-dev")
        }));
    }
    ...
    #[test]
    fn test_optimizer_preserves_root_stability_flags() {
        let mut pool = Pool::new();
        pool.add_stability_flag("vendor/dev-package", Stability::Dev);
        assert_ne!(
            pool.add_package(Package::new("vendor/dev-package", "dev-main")),
            0
        );

        let mut request = Request::new();
        request.require("vendor/dev-package", "dev-main");
        ...
        assert!(
            !optimized.packages_by_name("vendor/dev-package").is_empty(),
            "the optimized pool must retain dev packages allowed by root flags"
        );
    }
}
