...
#[test]
fn policy_exclusion_keeps_a_secure_candidate_selectable() {
    let mut pool = Pool::new();
    pool.add_package(pkg_with_requires(
        "shopware/core",
        "6.7.13.1",
        vec![("composer/composer", "^2.9.3")],
    ));
    let vulnerable = pool.add_package(pkg("composer/composer", "2.10.1.0"));
    pool.add_package(pkg("composer/composer", "2.10.2.0"));

    let mut request = Request::new();
    request.require("shopware/core", "=6.7.13.1");
    request.exclude(
        pool.entry(vulnerable)
            .and_then(PoolEntry::as_package)
            .unwrap()
            .clone(),
    );
    ...
    let result = Solver::new(&pool, &policy)
        .solve(&request)
        .expect("the secure Composer candidate should remain solvable");

    assert!(result
        .packages
        .iter()
        .any(|package| package.name == "composer/composer" && package.version == "2.10.2.0"));
    assert!(!result
        .packages
        .iter()
        .any(|package| package.name == "composer/composer" && package.version == "2.10.1.0"));
}
...
#[test]
fn test_broken_direct_dependency_does_not_select_unmentioned_replacer() {
    let mut pool = Pool::new();
    pool.add_package(pkg("a/a", "1.0.0"));
    pool.add_package(pkg_with_requires("b/b", "1.0.0", vec![("c/c", "1.*")]));
    let mut broken = pkg_with_requires("c/c", "1.0.0", vec![("x/x", "1.0")]);
    broken.replace.insert("a/a".into(), "1.0.0".into());
    pool.add_package(broken);
    pool.add_package(pkg_with_replaces(
        "d/d",
        "1.0.0",
        vec![("a/a", "1.0.0"), ("c/c", "1.0.0")],
    ));

    let policy = Policy::new();
    let solver = Solver::new(&pool, &policy);
    let mut request = Request::new();
    request.require("a/a", "1.*");
    request.require("b/b", "1.*");

    assert!(solver.solve(&request).is_err());
}

#[test]
fn test_transitively_required_replacer_can_replace_a_root_requirement() {
    let mut pool = Pool::new();
    pool.add_package(pkg("a/a", "1.0.0"));
    pool.add_package(pkg_with_requires("b/b", "1.0.0", vec![("c/c", "1.*")]));
    pool.add_package(pkg_with_replaces("c/c", "1.0.0", vec![("a/a", "1.0.0")]));
    ...
    request.require("a/a", "1.*");
    request.require("b/b", "1.*");
    let result = solver.solve(&request).expect("replacement is installable");

    assert!(result.packages.iter().any(|package| package.name == "c/c"));
    assert!(!result.packages.iter().any(|package| package.name == "a/a"));
}
...
fn test_no_install_replacer_of_missing_package() {
    ...
    let candidates =
        RepositoryUtils::filter_solver_candidates("b", vec![Arc::new(pkg_q)], |_| false);
    for candidate in candidates {
        pool.add_package_arc(candidate, None);
    }
...
#[test]
fn test_root_constraint_restricts_concrete_package_when_provider_satisfies_root() {
    let mut pool = Pool::new();
    let mut provider = pkg("foo/provider", "1.0.0");
    provider
        .provide
        .insert("foo/original".into(), "3.0.0".into());
    pool.add_package(provider);
    pool.add_package(pkg("foo/original", "1.0.0"));
    pool.add_package(pkg_with_requires(
        "foo/requirer",
        "1.0.0",
        vec![("foo/original", "1.0.0")],
    ));

    let policy = Policy::new();
    let solver = Solver::new(&pool, &policy);
    let mut request = Request::new();
    request.require("foo/original", "3.0.0");
    request.require("foo/provider", "1.0.0");
    request.require("foo/requirer", "1.0.0");
...
///
/// In Riff, requiring PHP before ext-foobar selects PHP 8.0.10, but requiring
