...
use riff_core::config::{Config, ConfigLoader, PreferredInstall, StoreAuths};
use std::env;
use std::fs;
...
fn test_config_defaults() {
    ...
    assert_eq!(config.github_protocols, vec!["https", "ssh"]);
...
fn test_config_loader_env_enabled() {
    ...
    let _test_var = EnvironmentGuard::set("COMPOSER_TEST_VAR", "test_value");
    let _vendor_dir = EnvironmentGuard::set("COMPOSER_VENDOR_DIR", "/custom/vendor");
...
fn test_build_config_with_project_dir() {
    ...
    fs::write(&manifest, json_content).unwrap();
...
fn test_config_env_overrides() {
    let _lock = ENVIRONMENT_LOCK.lock().unwrap();
    let _timeout = EnvironmentGuard::set("COMPOSER_PROCESS_TIMEOUT", "900");
    let _vendor_dir = EnvironmentGuard::set("COMPOSER_VENDOR_DIR", "/override/vendor");
...
fn test_authentication_config() {
    use riff_core::config::HttpBasicAuth;
