...
pub async fn execute(args: ShowArgs, context: &AppContext) -> Result<i32> {
    ...
    if args.platform {
        let packages = context.packages(&config)?;
        if args.format == "json" {
            ...
            for package in packages {
                println!("{:<30} {}", package.name, package.pretty_version());
            }
        }
        return Ok(0);
    }
    ...
    let composer = ComposerBuilder::new(working_dir.clone())
        ...
        .build()?;
    let repository_manager = composer.repository_manager;
    ...
    let displayed = if let Some(package_name) = &args.package {
        if !package_name.contains('*') && !show_latest {
            ...
            {
                eprintln!("Error: Package '{}' not found", package_name);
                return Ok(1);
            }
            show_single_package(
                &installed_packages,
...
async fn list_packages_with_latest(
    ...
    let latest_versions = if show_latest {
        fetch_latest_versions(&filtered, repository_manager).await
    } else {
        HashMap::new()
    };
    ...
    for pwl in packages {
        ...
        if args.name_only {
            ...
            if let Some(ref latest) = pwl.latest_version {
                ...
                println!(
                    "{}{} {:<15} {}{}",
                    linked_name, padding, version, desc, abandoned_marker
                );
            }
        }
    }
        ...
        if let Some(pkg) = package {
            ...
            if visited.contains(&dep_lower) {
                println!(
                    ...
                    prefix, branch, dep_name, version
                );
            } else {
                println!(
                    "{}{} {} {} ({})",
                    prefix, branch, dep_name, version, constraint
                );
