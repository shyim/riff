...
use riff_core::{
    ...
    json::{
        validate_parsed_composer_manifest, LockedPackage, ManifestValidation,
        ManifestValidationOptions, RiffLockfile, RiffManifest,
    },
};
...
pub async fn execute(args: ValidateArgs, context: &crate::CommandContext) -> Result<i32> {
    ...
    let composer = serde_json::from_str::<RiffManifest>(&content).ok();
...
fn validate_lock(
    ...
    let lock: RiffLockfile = match serde_json::from_str(&lock_content) {
        Ok(lock) => lock,
        Err(error) => {
...
fn missing_requirement_info(composer: &RiffManifest, lock: &RiffLockfile) -> Vec<String> {
    let mut errors = Vec::new();
...
mod tests {
    ...
    // testUnaccessibleFile. Riff checks the permission bits explicitly so
    // this contract remains deterministic even when the test runs as root.
    #[cfg(unix)]
    #[test]
    fn composer_validate_detects_an_unreadable_manifest() {
        use std::os::unix::fs::PermissionsExt;
