...
use riff_core::config::{AuthConfig, Config};
use riff_core::downloader::{ArchiveExtractor, ArchiveType, FileDownloader};
...
pub async fn execute(args: StatusArgs, context: &CommandContext) -> Result<i32> {
    ...
    if !composer_path.exists() {
        bail!(
            "Riff could not find a composer.json file in {}",
            working_dir.display()
        );
    }
