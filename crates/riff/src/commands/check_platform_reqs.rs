...
use riff_core::{
    config::Config,
    is_platform_package,
    json::{LockedPackage, RiffLockfile, RiffManifest},
    package::Package,
...
pub async fn execute(args: CheckPlatformReqsArgs, context: &CommandContext) -> Result<i32> {
    ...
    let composer: RiffManifest = serde_json::from_str(&composer_content)
...
fn read_lock(path: &Path) -> Result<RiffLockfile> {
...
fn collect_candidates(
    composer: &RiffManifest,
    packages: &[PackageMetadata],
    platform_packages: Vec<riff_core::Package>,
    ...
    // Composer's installed/lock repositories can contain synthetic platform
    // packages in tests and custom installations. Treat those as concrete
    // candidates just like runtime-probed platform packages.
    for package in packages
        .iter()
        .filter(|package| is_platform_package(&package.name))
    {
        candidates
            ...
            .push(Candidate {
                ...
                constraint: CandidateConstraint::Version(package.version.clone()),
            });
    }
