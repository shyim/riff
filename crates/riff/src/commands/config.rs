...
pub struct ConfigArgs {
    /// Show a configuration change without writing it or opening an editor
    #[usage(long)]
    pub dry_run: bool,
    ...
    #[usage(
        value_name = "SETTING-KEY",
        complete = crate::commands::completion::complete_config_key
    )]
...
pub async fn execute(args: ConfigArgs, context: &crate::CommandContext) -> Result<i32> {
    ...
    } else if !files.config_path.exists() && !args.dry_run {
        bail!(
    ...
    let mut document = if files.config_path.exists() {
        read_json_object(&files.config_path)?
    } else if args.global && args.dry_run {
...
fn write_json(path: &Path, value: &Value) -> Result<()> {
    ...
    riff_core::json::write_json_value(path, value, true)
...
fn merge_at_root(
    ...
    let target_child = target
        ...
        .or_insert_with(|| Value::Object(Map::new()));
    if key == "repositories" {
        merge_repositories(target_child, value);
    } else {
...
fn merge_repositories(target: &mut Value, incoming: &Value) {
    let (Value::Object(current), Value::Object(incoming)) = (&mut *target, incoming) else {
        *target = incoming.clone();
        return;
    };
    let mut merged = incoming.clone();
    for (name, repository) in std::mem::take(current) {
        merged.entry(name).or_insert(repository);
    }
    *current = merged;
}
...
pub(crate) fn completion_keys() -> Vec<&'static str> {
    ROOT_STRING_PROPERTIES
        .iter()
        .chain(ROOT_LIST_PROPERTIES)
        .chain(BOOL_CONFIG_KEYS)
        .chain(INTEGER_CONFIG_KEYS)
        .chain(STRING_CONFIG_KEYS)
        .chain(MULTI_CONFIG_KEYS)
        .chain(JSON_MERGE_KEYS)
        .chain(AUTH_KEYS)
        .copied()
        .chain([
            "preferred-install",
