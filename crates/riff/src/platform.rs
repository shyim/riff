...
use riff_core::{Platform, PlatformSnapshot, RuntimeContext};
...
struct ProbeOutput {
    ...
    #[serde(default, rename = "_riff_tracked_paths")]
...
#[derive(Debug, Clone)]
pub(crate) struct PhpPlatformDetector {
    runtime: RuntimeContext,
}

impl PhpPlatformDetector {
    pub(crate) fn from_sources(cli_php: Option<PathBuf>) -> Result<Self> {
        let php_binary = select_php_binary(
            cli_php,
            std::env::var_os("RIFF_PHP"),
            std::env::var_os("PHP_BINARY"),
        );
        let riff_binary = std::env::current_exe().context("Failed to locate riff")?;
        Ok(Self {
            runtime: RuntimeContext::new(php_binary, riff_binary),
        })
    }
    ...
    pub(crate) fn detect(&self) -> Result<Platform> {
        probe(&self.runtime.php_binary).map(Platform::from_snapshot)
    }
}

fn select_php_binary(
    cli: Option<PathBuf>,
    riff_php: Option<OsString>,
    php_binary: Option<OsString>,
) -> PathBuf {
    cli.or_else(|| riff_php.map(PathBuf::from))
...
fn run_probe(php_binary: &Path, timeout: Duration) -> Result<ProbeOutput> {
    ...
    if output.snapshot.php_version_id < MINIMUM_PHP_VERSION_ID {
        bail!(
            "riff requires PHP >= 7.2.5; {} reports {}",
            php_binary.display(),
...
fn platform_cache_dir() -> Option<PathBuf> {
    if std::env::var("RIFF_NO_PLATFORM_CACHE").is_ok_and(|value| value != "0") {
        return None;
    }
    Some(runtime_cache_dir().join("platform"))
}
...
mod tests {
    ...
    fn php_path_precedence_is_explicit_and_stable() {
        ...
        assert_eq!(
            select_php_binary(None, Some("riff-env".into()), Some("php-env".into())),
            PathBuf::from("riff-env")
        );
        assert_eq!(
    ...
    #[cfg(unix)]
    #[test]
    fn platform_detector_reuses_cached_probe() {
        ...
        let detector = PhpPlatformDetector::from_sources(Some(php)).unwrap();
    ...
    fn platform_cache_invalidates_changed_configuration() {
        ...
        executable(
            &php,
            &format!(
                "#!/bin/sh\nprintf x >> '{}'\nprintf '%s' '{{\"php_version\":\"8.3.1\",\"php_version_id\":80301,\"int_size\":8,\"zts\":false,\"debug\":false,\"ipv6\":true,\"extensions\":{{}},\"libraries\":{{}},\"_riff_tracked_paths\":[\"{}\"]}}'\n",
                count.display(),
    ...
    #[cfg(unix)]
    #[test]
    fn detector_rejects_an_unsupported_php_runtime() {
        let directory = tempfile::tempdir().unwrap();
        let php = directory.path().join("old-php");
        executable(
            &php,
            "#!/bin/sh\nprintf '%s' '{\"php_version\":\"7.1.33\",\"php_version_id\":70133,\"int_size\":8,\"zts\":false,\"debug\":false,\"ipv6\":true,\"extensions\":{},\"libraries\":{}}'\n",
        );

        let error = probe_with_timeout(&php, Duration::from_secs(1)).unwrap_err();
        assert!(error.to_string().contains("requires PHP >= 7.2.5"));
    }
}
