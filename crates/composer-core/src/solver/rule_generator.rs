...
impl<'a> RuleGenerator<'a> {
    ...
    fn add_package_rules(&mut self, package_id: PackageId) {
        ...
        for (dep_name, constraint) in sorted_requires {
            ...
            let direct_providers = self
                .pool
                .what_provides_direct_only(dep_name, Some(constraint));
            let has_direct = !direct_providers.is_empty();
            let dep_name_lower = dep_name.as_str().to_lowercase();
            let is_root_required = self.root_required_names.contains(&dep_name_lower);
