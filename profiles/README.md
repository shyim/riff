...
```sh
cargo build --profile profiling -p composer-rs-cli --bin composer-rs \
  --no-default-features
```
...
```sh
COMPOSER_CACHE_DIR=/tmp/composer-rs-profile-cache \
XDG_CACHE_HOME=/tmp/composer-rs-profile-cache \
valgrind --tool=callgrind --collect-jumps=yes \
  --callgrind-out-file=profiles/callgrind.hot-update.optimized.out \
  /tmp/composer-rs-target/profiling/composer-rs update --dry-run --no-scripts \
...
1.757 seconds; `composer-rs` averaged 20.6 milliseconds, an 85.5x mean speedup
